...
// Detecta se o app já está rodando como PWA instalada (standalone)
const isStandalone = () =>
...
export default function App() {
  ...
  // Ref para guardar o evento beforeinstallprompt (Android/Chrome)
  const deferredPromptRef = useRef(null)
  const [promptReady, setPromptReady]   = useState(false)

  // Verdadeiro se o app já está instalado (standalone) ou acabou de instalar
  const [instalado, setInstalado] = useState(() => isStandalone())

  // Toast de aviso quando o Chrome ainda não emitiu o prompt
  ...
  /*-------------------
    PWA – captura do prompt nativo
    Funciona em Android/Chrome quando a origem é tratada como
    segura (HTTPS ou via flag chrome://flags no celular).
  -------------------*/
  useEffect(() => {
    if (isStandalone()) return   // já instalado, não precisa ouvir

    const onBeforeInstall = (e) => {
      e.preventDefault()              // segura o mini-banner automático
      deferredPromptRef.current = e   // guarda para usar no clique
      setPromptReady(true)
    }
  ...
  // Fecha o toast automaticamente após 4 s
  useEffect(() => {
    if (!toast) return
  ...
  const handleInstalar = useCallback(async () => {
    if (!deferredPromptRef.current) {
      // O Chrome ainda não emitiu o prompt → provavelmente HTTP sem a flag
      setToast('⚠ Ative a flag no Chrome do celular para instalar via HTTP. Veja o README.')
      return
    }
    try {
    ...
    <>
      ...
      <header className="hud-topbar">
        ...
        <div className="hud-actions">
          {/*
            Mostra o botão se o app não está instalado.
            Fica em estado "aguardando" (opacity reduzida) até o
            Chrome emitir o beforeinstallprompt.
          */}
          {!instalado && (
            <button
              className={`botao_instalar_pwa${promptReady ? '' : ' pwa-btn-waiting'}`}
              id="btnInstalarPWA"
              onClick={handleInstalar}
              title={promptReady ? 'Instalar como app' : 'Aguardando permissão do navegador...'}
            >
              {promptReady ? '⬇ Instalar' : '⏳ Instalar'}
            </button>
          )}
